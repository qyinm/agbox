...
var legacySchemaFingerprint = []string{
	"actions", "candidate_corrections", "candidate_events", "candidates", "corrections",
	"events", "exports", "sessions", "source_cursors", "turns",
}
...
func classifyDatabase(path string) (databaseKind, error) {
	...
	for _, table := range legacySchemaFingerprint {
		if !tables[table] {
